...
fun PBJScreen(
    ...
    Column(
        ...
        if (pbjViewModel.pengajuans.value.isEmpty()) {
            EmptyPengajuanCard(
                title = stringResource(id = R.string.tidak_ada_pengajuan_pbj)
            )
        } else {
            LazyColumn {
                items(pbjViewModel.pengajuans.value) { pengajuan ->
                    PengajuanCard(pengajuan, onButtonClick = {
                        navController.navigate("${SipstisScreen.DetailPengajuanPBJ.name}/${pengajuan.id}")
                    })
                }
            }
        }
