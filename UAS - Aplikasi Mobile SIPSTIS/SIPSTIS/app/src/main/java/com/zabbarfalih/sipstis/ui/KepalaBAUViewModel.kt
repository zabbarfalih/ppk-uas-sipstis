...
private const val TAG = "KepalaBAUViewModel"

class KepalaBAUViewModel(
    ...
    fun fetchPengajuans() {
        viewModelScope.launch {
            try {
                ...
                val filteredPengajuan = allPengajuan.filter { it.status == StatusEnum.MENUNGGU_PERSETUJUAN }
                pengajuans.value = filteredPengajuan
            } catch (e: Exception) {
