...
fun PPKScreen(
    ...
    Column(
        ...
        if (ppkViewModel.pengajuans.value.isEmpty()) {
            EmptyPengajuanCard(
                title = stringResource(id = R.string.tidak_ada_pengajuan_ppk)
            )
        } else {
            LazyColumn {
                items(ppkViewModel.pengajuans.value) { pengajuan ->
                    PengajuanCard(pengajuan, onButtonClick = {
                        navController.navigate("${SipstisScreen.DetailPengajuanPPK.name}/${pengajuan.id}")
                    })
                }
            }
        }
